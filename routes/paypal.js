...
// Sets the risk transaction context for a merchant-generated tracking ID before
// the related PayPal transaction is created.
async function setTransactionContext(accessToken, trackingId, additionalData = []) {
  const response = await fetch(`${BASE_URL}/v1/risk/transaction-contexts`, {
    method: 'POST',
    headers: {
  ...
  if (!response.ok) {
    throw new Error(`PayPal setTransactionContext API error: ${JSON.stringify(data)}`);
  }
  return data;
}
...
router.post('/create-order', async (req, res) => {
  try {
    ...
    // PayPal requires this ID to be generated by the merchant, unique per
    // transaction, and reused for the associated payment calls.
    const trackingId = randomUUID().replaceAll('-', '').slice(0, 31);

    const account = req.session.checkoutAccount || {};
    const additionalData = [];
    // These keys must match the fields approved for your PayPal RaaS account.
    if (account.email) additionalData.push({ key: 'sender_email', value: account.email });
    if (account.firstName) additionalData.push({ key: 'sender_first_name', value: account.firstName });
    if (account.lastName) additionalData.push({ key: 'sender_last_name', value: account.lastName });

    // Set transaction context before creating the PayPal order. A risk-context
    // failure is returned to the caller so the order cannot proceed unlinked.
    await setTransactionContext(accessToken, trackingId, additionalData);
    ...
    // Keep the correlation ID server-side; the browser only needs the order ID.
    req.session.paypalTrackingIds = req.session.paypalTrackingIds || {};
    req.session.paypalTrackingIds[order.id] = trackingId;
...
router.post('/capture-order', async (req, res) => {
  try {
    ...
    const trackingId = req.session.paypalTrackingIds?.[orderId];
    if (!trackingId) {
      return res.status(400).json({ error: 'PayPal transaction context not found' });
    }
    ...
    if (data.status === 'COMPLETED') {
      ...
      delete req.session.paypalTrackingIds[orderId];
